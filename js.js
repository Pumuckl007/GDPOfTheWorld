...
economy.load = function() {
  ...
  request.onload = function(e) {
    economy.data = JSON.parse(e.currentTarget.response);
    economy.createData(economy.data);
    economy.map = $('#world-map').vectorMap({
      map: 'world_mill_en',
      series: {
        regions: [{
          scale: ['#661166', '#FF66AA'],
      ...
      onRegionTipShow: function(event, label, index){
        if(economy.areas[economy.year][index] && economy.areas[economy.year][index] !== -1 && economy.areas[economy.year][index] !== 1){
          label.html(
            '<b>'+economy.contries[index].name+'</b><br/>'+
            '<b>GDP: </b>$'+economy.areas[economy.year][index]
          );
        } else {
          label.html(
            '<b>'+economy.contries[index].name+'</b><br/>'+
...
economy.load();
economy.createData = function(data){
  var gdp = [];
  var countries = [];
  ...
  for(var key in data.GDP){
    if(economy.countryMap[key.toLowerCase()]){
      if(economy.countryMap[key.toLowerCase()].indexOf("&") !== -1){
        countries.push("ER");
        countries.push("ET");
        for(var year in data.GDP[key]){
          if(!gdp[economy.years[year]]){
            gdp[economy.years[year]] = [];
          }
          if(data.GDP[key][economy.years[year]]){
            gdp[economy.years[year]][countries.indexOf(countries.indexOf("ER"))] = data.GDP[key][economy.years[year]];
            if(!economy.yearMax[economy.years[year]]){
              economy.yearMax[economy.years[year]] = data.GDP[key][economy.years[year]];
            } else if(data.GDP[key][economy.years[year]] >= economy.yearMax[economy.years[year]]){
              economy.yearMax[economy.years[year]] = data.GDP[key][economy.years[year]];
            }
          } else {
            gdp[economy.years[year]][countries.indexOf(countries.indexOf("ER"))] = 1;
          }
          if(data.GDP[key][economy.years[year]]){
            gdp[economy.years[year]][countries.indexOf(countries.indexOf("ET"))] = data.GDP[key][economy.years[year]];
          } else {
            gdp[economy.years[year]][countries.indexOf(countries.indexOf("ET"))] = 1;
          }
        }
      } else {
        if(countries.indexOf(economy.countryMap[key.toLowerCase()]) == -1){
          countries.push(economy.countryMap[key.toLowerCase()]);
        }
        for(var year in economy.years){
          if(!gdp[economy.years[year]]){
            gdp[economy.years[year]] = [];
          }
          if(data.GDP[key][economy.years[year]]){
            gdp[economy.years[year]][countries.indexOf(economy.countryMap[key.toLowerCase()])] = data.GDP[key][economy.years[year]];
            if(!economy.yearMax[economy.years[year]]){
              economy.yearMax[economy.years[year]] = data.GDP[key][economy.years[year]];
            } else if(data.GDP[key][economy.years[year]] >= economy.yearMax[economy.years[year]]){
              economy.yearMax[economy.years[year]] = data.GDP[key][economy.years[year]];
            ...
            gdp[economy.years[year]][countries.indexOf(economy.countryMap[key.toLowerCase()])] = 1;
          }
        }
      }
    }
  ...
  economy.gdp = gdp;
  economy.countriesNames = countries;
  var areas = []
  for(var country in countries){
    for(var year in gdp){
      ...
      if(gdp[year][countries.indexOf(countries[country])]){
        areas[year][countries[country]] = gdp[year][countries.indexOf(countries[country])];
      } else {
        areas[year][countries[country]] = 0;
      }
    }
  }
  for(var name in economy.contries){
...
$('[data-slider]').on('change.fndtn.slider', function(){
  if(economy.first && this.dataset.options.indexOf("0.518134715") !== -1){
    economy.year = economy.years[Math.round(this.dataset.slider*1.93)];
    document.getElementById("year").innerHTML = economy.year.toString();
    if(!economy.mapObject){
      economy.mapObject = $('#world-map').vectorMap('get', 'mapObject');
    }
    if(economy.mapObject){
      if(economy.auto){
      ...
      economy.mapObject.series.regions[0].setValues(economy.areas[economy.year]);
      economy.mapObject.series.regions[0].scale.setMax(economy.yearMax[economy.year]);
    }
  } else if(economy.first){
    economy.max = Math.round(this.dataset.slider * 94851.36);
...
economy.countryMap = {};
economy.countryMap["eritrea & ethiopia"] = "ET|ER";
for(var key in economy.contries) {
    var value = economy.contries[key];
    if(value.name === "eritrea" | value.name === "ethiopia"){

    } else {
      economy.countryMap[value.name.toLowerCase().replace(
        "united states", "usa").replace(
          "indonesia","indonesia (& timor until '99)"
        ).replace(
          "haiti", "haïti"
        ).replace(
          "congo", "congo \'brazzaville\'"
        ).replace(
          "central african rep.", "centr. afr. rep."
        ).replace(
          "dem. rep. congo \'brazzaville\'", "dem. rep. congo"
        ).replace(
          ...
          "dem. rep. congo", "zaire (congo-kinshasa)"
        ).replace(
          "bosnia and herz.", "yugoslavia"
        ).replace(
          "united kingdom", "uk"
        ).replace(
          "russia", "f. ussr"
